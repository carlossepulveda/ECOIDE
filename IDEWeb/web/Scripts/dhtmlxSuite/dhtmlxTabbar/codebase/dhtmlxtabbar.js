...
dhtmlXTabBar.prototype={
    ...
    setSkin:function(a){
        ...
        var b=a.indexOf("dhx_sky")==0,c=a.indexOf("dhx_web")==0;
        ...
        if(b){
            ...
            if(this._s.expand){
                ...
                this._tabZone.firstChild.style.borderWidth=
                "0px 0px 0px 0px";
            ...
            var j=function(){
                ...
                b<6||
                (f.style[this._py]=parseInt(this._conZone.style[this._py])-3+"px",f.style[this._dx]=b-2+"px",g.style[this._py]=parseInt(this._conZone.style[this._py])-3+"px",g.style[this._dx]=b-(a?6:4)+(this._s.expand?2:0)+"px",h.style[this._dy]=parseInt(this._tabZone.style[this._dy])-3+"px",this._lineA.style[this._dx]=b-2+"px")
            };
    ...
    _setSizes:function(a,b){
        ...
        this._conZone.style[this._dx]=Math.max(0,d-(this._s.expand?0:
            2))+"px";
    ...
    _showScroll:function(a){
        if(!this._rows[a]._scroll){
            ...
            b[0].style.cssText="background-image:url("+this.imgUrl+this.skin+"/"+this.skin+"_scroll_left.gif); width:20px; height:21px;position:absolute;left:0px;z-index:990;top:"+
            c+"px;";
    ...
    _hideScroll:function(a){
        if(this._rows[a]._scroll)this._rows[a].removeChild(this._rows[a]._scroll[0]),this._rows[a].removeChild(this._rows[a]._scroll[1]),
    ...
    _setTabSizes:function(a){
        for(var b=this._a.offset,c=this._vMode?this._pxc:this._px,d=0;d<a.tabCount;d++){
            ...
            e.style.display!="none"&&(e.style[c]=b-(this._lastActive==
                e?this._a.select_shift:0)+"px",b+=e._size+this._a.margin)
        }
    },
    /**
     ***********************addTab************************* *****
     *a=el id de la pestaña
     *b= el nombre de la pestaña
     *c= el ancho si se fija de la pestaña
     *d= la posicion de donde se quiere meter la pestaña
     *e= fila vertical de donde ubicar la pestaña
     ************************************************************/
    ...
    //Metodo copiado y modificado para albergar otro parametro 
    //que servira de icono para la pestaña
    /**
     *a= id de la pestana
     *b= nombre de la pestana
     *c= ancho de la pestana
     *w= ruta del icono*/
    addTabIcon:function(a,b,c,w){
        !this.__skin&&dhtmlx.skin&&this.setSkin(dhtmlx.skin);
        var g=this._rows[0].tabCount;
        var h=this._createTabIcon(a,b,c=="*"?10:c||this._s._tabSize,w);
        this._addTab(this._rows[0],h,c,g);
        this._tabs[a]=h;
        this.cells(a).hide();
        this._checkScroll()
    },
    /******************************************************************************************
     *****************************************************************************************
     *este metodo tiene un agregado para poder eliminar del arraydeIds
     **/
    removeTab:function(a,b){
        ...
        if(c){
            ...
            this._content[a]&&this._content[a].parentNode&&
            this._content[a].parentNode.removeChild(this._content[a]);
    ...
    _createTab:function(a,b,c){
        ...
        e+="<IMG style='left:4px; top:4px; position:absolute;z-index:12;' src='"+this.imgUrl+this.skin+"/pdf.png' >";
        ...
        d.skin=this.skin;
        if(this._a.tab_color)d.style.backgroundColor=
            this._a.tab_color;
    ...
    //Metodo ajustado para agregar un icono en la pestaña
    //sumamos un parametro (f) que tiene la url de la imagen 16X16px que servira de icono
    _createTabIcon:function(a,b,c,w){
        
        var d=document.createElement("div");
        d.className="dhx_tab_element dhx_tab_element_inactive";
        var e="<span style='left:5px; padding-right:7px;'>"+b+"</span><div></div><div></div><div></div>";
        this._close&&(e+="<IMG style='"+this._pxc+":4px; "+this._py+(this.skin=="dhx_web"?":7":":4")+"px; position:absolute;z-index:11;' src='"+this.imgUrl+this.skin+"/close.png'>");
        //agregamos el icono
        e+="<IMG style='left:4px; top:4px; position:absolute;z-index:12;' src='"+this.imgUrl+"/"+w+"' >";
    ...
    _img_all:function(a){
        ...
        this._img(a.childNodes[3],c+1,this._px,parseInt(a.style[this._dx])-
    ...
    _addTab:function(a,b,c,d){
        ...
        if(a.tabCount)var f=a.childNodes[a.tabCount-1],e=parseInt(f.style[this._s.align?this._pxc:this._px])+parseInt(f._size)+this._a.margin;
    ...
    adjustTabSize:function(a,b){
        ...
        if(!b)a.style.fontWeight="bold",a.childNodes[3].style[this._dx]=a.style[this._dx]="10px",b=a[this._s._vMode?"scrollHeight":"scrollWidth"]+
            10+(this._close?20:0),a.style.fontWeight="";
    ...
    _setTabInActive:function(a){
        ...
        c=a.getAttribute("tab_id"),d=this._s.align?this._pxc:this._px;
    ...
    _setTabActive:function(a,b){
        if(!a)return!0;
        var c=a.getAttribute("tab_id"),d=this._lastActive?this._lastActive.getAttribute("tab_id"):
        null,e=this._getSkin(a);
    ...
    _setTabTop:function(a){
        for(var b=
            this._rows.length-1,c=0;c<this._rows.length;c++)if(this._rows[c]==a.parentNode){
            var d=this._rows[c];
    ...
    _parseXML:function(a,b,c,d,e){
        ...
        for(var j=0;j<h.length;j++)for(var k=e.doXPath("./tab",h[j]),l=0;l<k.length;l++){
            var p=k[l].getAttribute("width"),q=a._getXMLContent(k[l]),m=k[l].getAttribute("id");
            a.addTab(m,q,p,"",j);
            k[l].getAttribute("selected")&&
            (f=m);
            if(a._hrfmode)a.setContentHref(m,k[l].getAttribute("href"));else for(var o=0;o<k[l].childNodes.length;o++){
    ...
    /******************************************************************************************
     ******************************************************************************************
     ***********************CODIGO AGREGADO EL 16/12/2011 By Wilson****************************
     ******************************************************************************************
     ******************************************************************************************/
    setContainTab:function(id,editor){
        this._contenido[id.toUpperCase()]=editor;
    },
    //esta funcion permite revisar
    containTab:function(id){
        for(var key2 in this._contenido){
            if(key2==id.toUpperCase()){
                return true;
            }
        }
        return false;
    },
    removeContainTab:function(id){
        delete this._contenido[id.toUpperCase()];
    },
    getEditorAce:function(id){
        for(var key in this._contenido){
            if(key==id.toUpperCase())
                return this._contenido[key];
        }
        return null;
    },
    /*******************************************************************************************
    ...
    tabWindow:function(a){
        return this._content[a]?this._content[a]._frame.contentWindow:
        null
    },
    ...
    setContentHref:function(a,b){
        ...
        switch(this._hrfmode){
            case "iframes":
                ...
                break;
            case "iframe":case "iframes-on-demand":
                this.cells(a)._delay=["url",b,!1];
                break;
            case "ajax":
                var c=this.cells(a),d=this;
