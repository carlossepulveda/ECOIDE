...
module.exports = Model = (db, options) ->
  ...
  refreshReapingTimeout = (docName, pathFile) ->
    doc = docs[docName]
    if pathFile isnt null && pathFile isnt undefined
       #console.warn "deberia estar guardando    "+"../../../"+pathFile+"    "+JSON.stringify doc.snapshot
       console.warn "********************++++++++++*****+TIIPOOOOOO:   "+doc.type.name
       texto=doc.snapshot
       if doc.type.name is "json"
          texto=JSON.stringify doc.snapshot
       fs = require('fs')
       fs.writeFile __dirname + '/../../../../'+pathFile, texto
  ...
  @delete = (docName, callback) ->
    console.warn "llego a eliminar un documenttoooooo"
    doc = docs[docName]
  ...
  @applyOp = (docName, opData, callback, pathFile) ->
